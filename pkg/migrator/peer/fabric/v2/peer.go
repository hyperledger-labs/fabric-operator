...
func (m *Migrate) MigrationNeeded(instance metav1.Object) bool {
	...
	for _, cont := range dep.Spec.Template.Spec.Containers {
		if strings.ToLower(cont.Name) == "dind" {
			// DinD container found, instance is not at v2
...
func (m *Migrate) UpdateConfig(instance metav1.Object, version string) error {
	log.Info("Updating config to v2")
	cm, err := m.ConfigMapManager.GetCoreConfig(instance.(*current.IBPPeer))
	if err != nil {
	...
	// As per this PR #2165, VM and Ledger structs been added to Peer. endpoint is not required for v2 peer as there is no DinD
	core.VM.Endpoint = ""
	...
	if currentVer.EqualWithoutTag(ver.V2_4_1) || currentVer.GreaterThan(ver.V2_4_1) {
		trueVal := true
		core.Peer.Gateway = v2peer.Gateway{
			Enabled:            &trueVal,
...
// Updates required from v1.4 to v2.x:
// - External builders
// - Limits
// - Install timeout
// - Implicit collection dissemination policy
func configHasBeenUpdated(core *v2config.Core) bool {
