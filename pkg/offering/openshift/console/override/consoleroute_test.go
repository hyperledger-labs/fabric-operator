...
var _ = Describe("Openshift Console Route Overrides", func() {
	var (
		route     *routev1.Route
	...
	Context("create", func() {
		It("appropriately overrides the respective values", func() {
			err := overrider.ConsoleRoute(instance, route, resources.Create)
			Expect(err).NotTo(HaveOccurred())

			Expect(route.Name).To(Equal(fmt.Sprintf("%s-console", instance.Name)))
			Expect(route.Spec.Host).To(Equal("testNS-route1-console.test-domain"))
