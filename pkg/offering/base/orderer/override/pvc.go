...
func (o *Override) CreatePVC(instance *current.IBPOrderer, pvc *corev1.PersistentVolumeClaim) error {
	...
	if storage != nil {
		ordererStorage := storage.Orderer
		if ordererStorage != nil {
			if ordererStorage.Class != "" {
				pvc.Spec.StorageClassName = &ordererStorage.Class
			}
			if ordererStorage.Size != "" {
				quantity, err := resource.ParseQuantity(ordererStorage.Size)
				if err != nil {
					return err
				}
