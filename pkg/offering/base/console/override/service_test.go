...
var _ = Describe("Base Console Service Overrides", func() {
	var (
	...
	BeforeEach(func() {
		...
		service, err = util.GetServiceFromFile("../../../../../definitions/console/service.yaml")
	...
	Context("create", func() {
		It("overrides values based on spec", func() {
			...
			By("instance spec proxying to ture", func() {
				Expect(*instance.Spec.Proxying).To(Equal(true))
			})

			By("setting console node port", func() {
				Expect(service.Spec.Ports[0].NodePort).To(Equal(instance.Spec.NetworkInfo.ConsolePort))
			})
		})
