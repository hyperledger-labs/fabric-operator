...
func (o *Override) CreatePVC(instance *current.IBPConsole, pvc *corev1.PersistentVolumeClaim) error {
	...
	if storage != nil {
		consoleStorage := storage.Console
		if consoleStorage != nil {
			if consoleStorage.Class != "" {
				if consoleStorage.Class == "local" {
					class := "manual"
					pvc.Spec.StorageClassName = &class
				} else {
					pvc.Spec.StorageClassName = &consoleStorage.Class
				}
			}

			if consoleStorage.Size != "" {
				quantity, err := resource.ParseQuantity(consoleStorage.Size)
				if err != nil {
					return err
				}
