...
var _ = Describe("Base Console PVC Overrides", func() {
	var (
	...
	BeforeEach(func() {
		...
		pvc, err = util.GetPVCFromFile("../../../../../definitions/console/pvc.yaml")
	...
	Context("create", func() {
		It("overrides values based on spec", func() {
			...
			By("setting storage class", func() {
				Expect(pvc.Spec.StorageClassName).To(Equal(&instance.Spec.Storage.Console.Class))
			})

			By("setting requested storage size", func() {
				expectedRequests, err := resource.ParseQuantity(instance.Spec.Storage.Console.Size)
				Expect(err).NotTo(HaveOccurred())
		...
		It("sets class to manual if spec used local", func() {
			instance.Spec.Storage.Console.Class = "local"
			err := overrider.PVC(instance, pvc, resources.Create)
			Expect(err).NotTo(HaveOccurred())
		...
		It("returns an error if invalid value for size is used", func() {
			instance.Spec.Storage.Console.Size = "10x"
			err := overrider.PVC(instance, pvc, resources.Create)
