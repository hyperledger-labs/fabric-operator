...
var _ = Describe("Role Binding manager", func() {
	var (
		mockKubeClient *mocks.Client
		manager        *rolebinding.Manager
	...
	BeforeEach(func() {
		mockKubeClient = &mocks.Client{}
		manager = &rolebinding.Manager{
			RoleBindingFile: "../../../../definitions/ca/rolebinding.yaml",
	...
	Context("reconciles the  instance", func() {
		It("does not try to create role binding if the get request returns an error other than 'not found'", func() {
		...
		When("role binding does not exist", func() {
			...
			It("returns an error if fails to load default config", func() {
				manager.RoleBindingFile = "bad.yaml"
				err := manager.Reconcile(instance, false)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("no such file or directory"))
			})

			It("returns an error if override role binding value fails", func() {
				manager.OverrideFunc = func(v1.Object, *rbacv1.RoleBinding, resources.Action) error {
			...
			It("returns an error if the creation of the role binding fails", func() {
