...
var _ = Describe("Role manager", func() {
	...
	BeforeEach(func() {
		mockKubeClient = &mocks.Client{}
		manager = &role.Manager{
			RoleFile: "../../../../definitions/ca/role.yaml",
	...
	Context("reconciles the  instance", func() {
		It("does not try to create role if the get request returns an error other than 'not found'", func() {
		...
		When("role does not exist", func() {
			...
			It("returns an error if fails to load default config", func() {
				manager.RoleFile = "bad.yaml"
				err := manager.Reconcile(instance, false)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("no such file or directory"))
			})

			It("returns an error if override role value fails", func() {
				manager.OverrideFunc = func(v1.Object, *rbacv1.Role, resources.Action) error {
			...
			It("returns an error if the creation of the role fails", func() {
				errMsg := "unable to create role"
