...
var _ = Describe("ConfigMap manager", func() {
	var (
	...
	BeforeEach(func() {
		mockKubeClient = &mocks.Client{}
		manager = &configmap.Manager{
			ConfigMapFile: "../../../../definitions/console/console-configmap.yaml",
	...
	Context("reconciles the configmap instance", func() {
		It("does not try to create configmap if the get request returns an error other than 'not found'", func() {
		...
		When("configmap does not exist", func() {
			...
			It("returns an error if fails to load default config", func() {
				manager.ConfigMapFile = "bad.yaml"
				err := manager.Reconcile(instance, false)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("no such file or directory"))
			})

			It("returns an error if override configmap value fails", func() {
				manager.OverrideFunc = func(v1.Object, *corev1.ConfigMap, resources.Action, map[string]interface{}) error {
			...
			It("returns an error if the creation of the ConfigMap fails", func() {
				errMsg := "unable to create configmap"
			...
			It("does not return an error on a successfull ConfigMap creation", func() {
				err := manager.Reconcile(instance, false)
