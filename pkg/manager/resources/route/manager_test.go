...
var _ = Describe("Route manager", func() {
	...
	BeforeEach(func() {
		mockKubeClient = &mocks.Client{}
		manager = &route.Manager{
			RouteFile: "../../../../definitions/ca/route.yaml",
	...
	Context("reconciles the route instance", func() {
		It("does not try to create route if the get request returns an error other than 'not found'", func() {
		...
		When("route does not exist", func() {
			...
			It("returns an error if fails to load default config", func() {
				manager.RouteFile = "bad.yaml"
				err := manager.Reconcile(instance, false)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("no such file or directory"))
			})

			It("returns an error if override route value fails", func() {
				manager.OverrideFunc = func(v1.Object, *routev1.Route, resources.Action) error {
			...
			It("returns an error if the creation of the Route fails", func() {
				errMsg := "unable to create route"
			...
			It("does not return an error on a successfull route creation", func() {
				err := manager.Reconcile(instance, false)
