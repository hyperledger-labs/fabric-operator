...
type Manager struct {
	...
	OverrideFunc func(v1.Object, *networkingv1.Ingress, resources.Action) error
...
func (m *Manager) Reconcile(instance v1.Object, update bool) error {
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			log.Info(fmt.Sprintf("Creating ingress '%s'", name))
	...
	if update {
		if m.OverrideFunc != nil {
			log.Info(fmt.Sprintf("Updating ingress '%s'", name))
...
func (m *Manager) GetIngressBasedOnCRFromFile(instance v1.Object) (*networkingv1.Ingress, error) {
	ingress, err := util.GetIngressFromFile(m.IngressFile)
	if err != nil {
		log.Error(err, fmt.Sprintf("Error reading ingress configuration file: %s", m.IngressFile))
		return nil, err
	}
...
func (m *Manager) BasedOnCR(instance v1.Object, ingress *networkingv1.Ingress) (*networkingv1.Ingress, error) {
	if m.OverrideFunc != nil {
		err := m.OverrideFunc(instance, ingress, resources.Create)
		if err != nil {
			return nil, errors.Wrap(err, "failed during ingress override")
		}
	}
...
func (m *Manager) UpdateIngressClassName(name string, instance v1.Object) error {
	...
	log.Info("Waiting for ingress resource to be ready", "ingress", name)
